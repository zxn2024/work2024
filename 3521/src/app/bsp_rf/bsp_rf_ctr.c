...
void rf_ctr_ch_close_process(uint8_t sys)
{
	...
	static BOOL_E   first_in_flag[RF_SYS_NUM] = {B_TRUE, B_TRUE, B_TRUE };
	...
	//首次进入初始化起始时间
	if( B_TRUE == first_in_flag[sys] )
	{
		first_in_flag[sys]   = B_FALSE;
...
static void rf_real_gain_cal(uint8_t sys)
{
	...
	att_num = ( att_num < 0 ) ? 0 : att_num;
	att_num = (att_num >= pt->tAtt.constAttNum) ? pt->tAtt.constAttNum :  att_num;
