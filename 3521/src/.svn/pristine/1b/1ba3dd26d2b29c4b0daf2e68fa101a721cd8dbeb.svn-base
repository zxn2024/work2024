 ...
 * 输  出： -
 */
static Status_t switch_ctl(struct __Switch_spdt_TypeDef * pSwitchHandle, SW_STATE_E sw_ctl)
{
    // 参数检查
    ...
    if(sw_ctl == SW_CH1)
    {
        ...
        hi_gpio_write(pSwitchHandle->hw_config.switch_ctl_port, pSwitchHandle->hw_config.switch_ctl_pin, LEVEL_L);
    }
    else if(sw_ctl == SW_CH2)
    {
        pSwitchHandle->switch_ctl_stat = sw_ctl;
        hi_gpio_write(pSwitchHandle->hw_config.switch_ctl_port, pSwitchHandle->hw_config.switch_ctl_pin, LEVEL_H);
    }
    else
        return STAT_ERROR;
 ...
 * 输  出： -
 */
static Status_t switch_open(struct __Switch_spdt_TypeDef * pSwitchHandle)
{
    // 参数检查
    if(pSwitchHandle == NULL)
        return STAT_ERROR;
