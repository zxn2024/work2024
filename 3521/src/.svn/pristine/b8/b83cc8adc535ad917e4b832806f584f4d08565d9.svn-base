...
uint8_t mcp_mrc_mu_parase_msg(MCP_MSG_T *mcp_msg)
{
    ...
    if(mcp_msg->dev_id == 0)                                   // id为0表示为从设备发起给主设备的包,仅支持从机发起的上报,其他丢掉
    {
        ru_index = 0;
        if(mcp_msg->cmd_type == MCP_CMD_TYPE_RPT  && mcp_msg->vp_sign == VP_SIGN_SEND )
        {
        ...
        {
            // 其他命令类型,丢掉
            ret = MCP_RESP_SIGN_OTHER_ERROR;
        }

    }else if((mcp_msg->dev_id > 0 && mcp_msg->dev_id <= BLE_CONNECT_MAX_NUM) )    // id大于0,小于等于从机数量,主设备发起给从设备的包的回包
    {
        ru_index = get_mrc_ru_device_index_by_conn_id(mcp_msg->dev_id);

        if( mcp_msg->cmd_type == MCP_CMD_TYPE_QUERY && mcp_msg->vp_sign == VP_SIGN_REPLY)
        {
            while ((Num + 3) < mcp_msg->rx_buf_len)
            {
                data_block_len = *(mcp_msg->ptr_data_unit);
    ...
    {
        rtt_printf(RTT_BLE, RTT_CTRL_TEXT_BRIGHT_YELLOW "Mcp dev_id error:%d\r\n", mcp_msg->dev_id);
        return MCP_RESP_SIGN_OTHER_ERROR;
    }
