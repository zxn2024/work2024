...
#define CELL_INFO_QUARY_CNT (S_TO_TICKS(5))   // 间隔固定时间查询小区信息, 5S  1次
#define AT_CMD_MAX_ERROR_CNT (3)              // AT指令无回复/错误回复最大次数
#define AT_CMD_MAX_REBOOT_CNT (1)             // REBOOT最大次数
#define AT_RETRY (5)                          // AT命令发送的最大次数
#define AT_MAX_NET_INFO_INVALD_CNT (5)        // ECELL查询小区信息 无效的最大次数
#define SCAN_MAX_TIMEOUT (20*1000)            // CFUN1搜网 最大的超时时间 20S  
/*---------------------- 全局变量 --------------------------------------*/
static uint8_t index = 0;  //用于搜网结果解析后存储的索引
uint8_t g_cmd_type = MODEM_CMD_END;
uint8_t set_frq_point_buf[64] = {0}; // 锁定频点用的缓存区域, 锁频点需要将频点数据先复制到该区域

char buffer_freq[FREQ_BUF_SIZE]; //频点信息存储buffer

int8_t max_index_lte = 0;  //LTE搜网结果中，含有最大的rsrp的信息存储索引
int8_t max_index_nr = 0;  //NR搜网结果中，含有最大的rsrp的信息存储索引
 ...
 * @Note   :
 */
static Status_t modem_send_AT_cmd(uint8_t cmd_type, int param1, int param2, int param3, int param4, int param5)
{
    ...
    if (remain_time_ms > 0)
    {
        ...
        case MODEM_CMD_SET_ERAT:        // 设置网络制式 NR LTE NR+LTE
        case MODEM_CMD_QUERY_BAND:      // 查询BAND及频点  结果仅从串口打印,未存储
        case MODEM_CMD_SET_EL1CFG:      // 设置物理层参数,时隙配比
        case MODEM_CMD_SET_ETIMEOFFSET: // 设置NR2通道的时间偏移量,并跟踪和输出同步信号
        case MODEM_CMD_SET_EGPIOSIGNAL: // 设置GPIO信号输出(配单通或TDD)
        case MODEM_CMD_SET_EGPIOADJ:    // 设置GPIO组对应的信号调整量
        case MODEM_CMD_SET_LOCK_EBAND:  // 锁定BAND
	    case MODEM_CMD_SET_UNLOCK_EBAND: // 解锁BAND			
        case MODEM_CMD_SET_CFUN_0: // 设置运行模式0
		case MODEM_CMD_SET_CFUN_1: // 设置运行模式1
        ...
        case MODEM_CMD_QUARY_ECELL: // 查询小区信息
            ...
            if ((pStart != NULL) && (pEnd != NULL))
            {
                ...
                if (pStart != NULL)
                {
					...
					if(STAT_ERROR==is_valid_lte_net_info())
					{
					   if(++hModem.LTE_net_info.lte_net_invalid_cnt> AT_MAX_NET_INFO_INVALD_CNT) 
						{
					     hModem.LTE_net_info.lte_net_invalid_cnt=0;
						 hModem.LTE_net_info.LTE_Sync=SCAN_UNSYNC;
						}
					}else
					{
					   hModem.LTE_net_info.lte_net_invalid_cnt=0;
					   hModem.LTE_net_info.LTE_Sync=SYNC_SYNC; 
					}					
                }
                ...
                if (pStart != NULL)
                {
					...
					if(STAT_ERROR==is_valid_nr_net_info())
					{
					   if(++hModem.NR_net_info.nr_net_invalid_cnt> AT_MAX_NET_INFO_INVALD_CNT) 
						{
						 hModem.NR_net_info.nr_net_invalid_cnt=0; 
						 hModem.NR_net_info.NR_Sync=SCAN_UNSYNC;
						}
					}else
					{
					   hModem.NR_net_info.nr_net_invalid_cnt=0; 
					   hModem.NR_net_info.NR_Sync=SYNC_SYNC;
					}	
                }
                RetValue = STAT_OK;
            }
...
void modem_init(void)
{
    ...
    modem_reboot_or_reset_cleardata(); 	
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_REBOOT, 0, 0, 0, 0, 0); // 发送AT指令
    return RetValue;
}
...
void vModemProcess(void)
{
    ...
    case MODEM_REBOOT:     //REBOOT后不管成功还是失败都复位重上电
		...
		{
		    rtt_printf(RTT_MODEM, "modem_reboot fail\r\n");
		}

        //vTaskDelay(100); // 如果reboot发送多次，则命令之前添加延时
    ...
    if (bitsTimeCntEventGroup & BIT_SOCKET_TIME_CELLINFO_QUARY)
    {
        ...
        if ((((hModem.network == LTE) && (hModem.LTE_net_info.LTE_Sync == SYNC_SYNC)) 
			|| ((hModem.network == NR) && (hModem.NR_net_info.NR_Sync == SYNC_SYNC)) 
		    || ((hModem.network == LTE_NR) && ((hModem.LTE_net_info.LTE_Sync == SYNC_SYNC) || (hModem.NR_net_info.NR_Sync == SYNC_SYNC))))
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_QUERY_EGMR, 0, 0, 0, 0, 0); // 发送AT指令
    return RetValue;
}
/***********************************************************
 *函数功能: 获取AP版本
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_QUERY_EGAPR, 0, 0, 0, 0, 0); // 发送AT指令
    return RetValue;
}
/***********************************************************
 *函数功能: 获取硬件版本
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_QUERY_EGHWR, 0, 0, 0, 0, 0); // 发送AT指令
/***********************************************************
 *函数功能: 获取软件版本
 *参数说明: 无
 *返 回 值: STAT_OK /STAT_ERROR
...
/*********************************************************************
 * 函 数 名：void vModemRun(void)
 * 功能描述：modem业务处理函数,用于器件初始化,modem运行配置，状态监控,错误处理等
 * 参数说明：无
 * 函数输出：无
 *********************************************************************/
void modem_run(void)
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_ERAT, network_format, 0, 0, 0, 0); // 发送AT指令
    return RetValue;
}
...
Status_t modem_lock_band(uint8_t network_channel, uint8_t band, char *arfcn)
{
    ...
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_LOCK_EBAND, network_channel, band, 0, 0, 0);
    return RetValue;
}
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_UNLOCK_EBAND, network_channel, band, 0, 0, 0); // 发送AT指令
    return RetValue;
}
/***********************************************************
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_CFUN_0, 0, 0, 0, 0, 0); // 发送AT指令
...
Status_t modem_start_scan()
{
    ...
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_CFUN_1, 0, 0, 0, 0, 0); // 发送AT指令
    return RetValue;
}
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_ECELLGPIO_LTE, freq, pci, band, 0, 0); // 发送AT指令
    return RetValue;
}
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_ECELLGPIO_NR, freq, pci, band, 0, 0); // 发送AT指令
    return RetValue;
}
...
Status_t modem_set_channel_output_mode(uint8_t gpio_group, gpio_sync_mode_E gpio_mode_up, gpio_sync_mode_E gpio_mode_dw)
{
    ...
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_EGPIOSIGNAL, gpio_group, gpio_mode_up, gpio_mode_dw, 0, 0); // 发送AT指令
    return RetValue;
}
...
Status_t modem_set_gpio_timing_config(uint8_t gpio_group, timing_config_t gpio_timing, uint8_t cfg_mode)
{
    ...
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_EL1CFG, gpio_group, cfg_mode, 0, 0, 0); // 发送AT指令
    return RetValue;
}
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_EGPIOADJ, gpio_group, ul_rising, ul_falling, dl_rising, dl_falling);
    return RetValue;
}
/***********************************************************
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_SET_ETIMEOFFSET, time_offset, 0, 0, 0, 0); // 发送AT指令
    return RetValue;
}
...
Status_t modem_refresh_net_info(void)
{
    ...
    {
        RetValue = modem_send_AT_cmd(MODEM_CMD_QUARY_ECELL, 0, 0, 0, 0, 0); // 发送AT指令
    }
    return RetValue;
}
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_QUERY_BAND, 0, 0, 0, 0, 0); // 发送AT指令
    return RetValue;
}
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_QUERY_CFUN, 0, 0, 0, 0, 0); // 发送AT指令
    return RetValue;
}
/***********************************************************
 *函数功能: 获取模块的网络制式
...
{
    Status_t RetValue = STAT_ERROR;
    RetValue = modem_send_AT_cmd(MODEM_CMD_QUERY_ERAT, 0, 0, 0, 0, 0); // 发送AT指令
    return RetValue;
}

#if 0
/***********************************************************
 *函数功能: 初始化上层应用配置模块工作所需的参数  驱动测试用
 ********************************************************/
...
void ModemConfig(void)
{
	...
	static uint32_t init_start_tick_cfun1 = 0;
    ...
    case CONFIG_UNLOCKBAND: // 解锁所有频点
        ...
        if (RetValue == STAT_OK)
        {
            ...
            rtt_printf(RTT_MODEM, "modem_unlock_band ok!\r\n");
            break;
        }
        else
        {
            rtt_printf(RTT_MODEM, "modem_unlock_band fail!\r\n");
            vTaskDelay(xTicksToWait);
        }
    ...
    case CONFIG_CFUN1: // 设置全功能模式 搜网
        ...
        if (RetValue == STAT_OK)
        {
			...
			init_start_tick_cfun1 = hi_get_tick();        //扫描开始计时
    ...
    case CONFIG_GETNETINF: // 获取搜网结果 hModem.scan_result.NR_scan_pci_num    hModem.scan_result.LTE_scan_pci_num
        ...
        {
	    	if ((hi_get_tick() - init_start_tick_cfun1) > SCAN_MAX_TIMEOUT)
			{
				init_start_tick_cfun1 = 0;
				rtt_printf(RTT_MODEM, RTT_CTRL_TEXT_BRIGHT_RED "Modem cfun1 scan timeout!!\r\n");
...
Status_t  is_valid_lte_net_info(void)
 {	
    if((hModem.LTE_net_info.band==0)&&(hModem.LTE_net_info.pci==0)&&(hModem.LTE_net_info.freq==0))	 
	{        
        return STAT_ERROR; 
    }else
    {
...
Status_t  is_valid_nr_net_info(void)
 {	
    if((hModem.NR_net_info.band==0)&&(hModem.NR_net_info.pci==0)&&(hModem.NR_net_info.freq==0))	 
	{        
        return STAT_ERROR; 
    }else
    {
