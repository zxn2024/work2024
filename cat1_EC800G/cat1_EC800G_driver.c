...
void cat1_omc_para_set(omc_t *omc_param) // flash参数读取放到flash配置读取功能里调用
{
    ...
    hCat1.socketConfig[0].tcp_or_udp = TCP;
    if(hCat1.socketConfig[0].ip_type != IPV4 && hCat1.socketConfig[0].ip_type != IPV6 && hCat1.socketConfig[0].ip_type != IPV4V6 )
        hCat1.socketConfig[0].ip_type = IPV4;
    ...
    if (remain_time_ms > 0)
    {
        ...
        case CAT1_CMD_QUERY_SIM_ID:
            ...
            if ((pStart != NULL) && (pEnd != NULL))
            {
                ...
                {
                    pStart += strlen("+QCCID: ");
                    memcpy(hCat1.ucaSimNum, pStart, ((pEnd - pStart) > 30) ? 30 : (pEnd - pStart));
