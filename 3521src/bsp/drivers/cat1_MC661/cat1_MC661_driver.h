 ...
 */
#define CAT1_HW_TEST (0)  //正常是0，测试1
/*
 * @brief 连接类型定义
 */
#define UDP (1)
...
typedef enum
{
    ...
    CAT1_CMD_SET_AUTH, // 设置 PDP 鉴权参数

    CAT1_CMD_SET_QIACT,     // 激活PDP场景
    CAT1_CMD_QUARY_QIACT,   // 查询激活PDP场景？？
    CAT1_CMD_SET_QIDEACT,   // 反激活PDP场景
    ...
    CAT1_CMD_SET_MIPCALL,   // 创建无线链路
	CAT1_CMD_SET_DEMIPCALL,  //断开无线链路
    CAT1_CMD_QUERY_MIPCALL, // 查询PDP
    CAT1_CMD_SET_MIPSETS0,  // 设置推送数据大小和超时
    CAT1_CMD_SET_MIPSETS1,  // 设置推送数据大小和超时

    CAT1_CMD_SET_MIPDSETS0, // 设置输出数据大小和超时
    CAT1_CMD_SET_MIPDSETS1, // 设置输出数据大小和超时
    CAT1_CMD_SET_GTSET,     // 设置hex模式
    CAT1_CMD_QUERY_DATA,    // 从缓冲区读取数据
    CAT1_CMD_SET_MIPPUSH0,  // 将缓冲区的数据推送到协议栈
    CAT1_CMD_SET_MIPPUSH1,  // 将缓冲区的数据推送到协议栈

    CAT1_CMD_QUERY_CSQ, // 查询模块接收到的信号强度指示<rssi>和信道的误码率<ber>
...
typedef struct
{
    cat1_mc661_hw_config_t cat1_hw_config; // 硬件配置,串口,GPIO
...
/*---------------------- 全局变量  ---------------------------------------*/
extern cat1_mc661_T hCat1; // CAT1结构体
